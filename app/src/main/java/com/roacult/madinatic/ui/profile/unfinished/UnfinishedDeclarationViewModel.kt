...
import com.roacult.domain.usecases.profile.UserUnfinishedDeclaration
...
class UnfinishedDeclarationViewModel(
    private val userUnfinishedDeclaration: UserUnfinishedDeclaration,
    private val gson: Gson,
    private val stringProvider: StringProvider
)  : BaseViewModel<UnfinishedDeclarationState>(UnfinishedDeclarationState()) ,
    ...
    override fun loadPage(
        page: Int,
        callback: (Either<DeclarationFailure, DeclarationPage>) -> Unit
    ) {
        setState { copy(declarationState = Loading()) }
        scope.launchInteractor(userUnfinishedDeclaration, page) {
            callback(it)
            it.either({
                val msg = when (it) {
...
data class UnfinishedDeclarationState(
